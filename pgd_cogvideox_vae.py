...
def load_image(path: str, size: int = 480) -> torch.Tensor:
    """Load image, resize, return tensor in [-1, 1] with shape (1, 3, 1, H, W) for video VAE."""
    img = Image.open(path).convert("RGB").resize((size, size))
    arr = np.array(img).astype(np.float32) / 127.5 - 1.0
    # (1, 3, 1, H, W) — single-frame "video"
    return torch.from_numpy(arr).permute(2, 0, 1).unsqueeze(0).unsqueeze(2)


def tensor_to_numpy_image(t: torch.Tensor) -> np.ndarray:
    """(1,3,1,H,W) or (1,3,H,W) or (3,H,W) in [-1,1] → (H,W,3) in [0,1]."""
    if t.dim() == 5:
        t = t[0, :, 0]  # (3, H, W)
    elif t.dim() == 4:
        t = t[0]
...
def latent_to_rgb_channels(z: torch.Tensor) -> np.ndarray:
    ...
    z: (1, C, 1, h, w) → returns (h, w, 3) numpy array."""
    z3 = z[0, :3, 0].cpu().float()  # (3, h, w)
    for c in range(3):
...
def latent_to_rgb_pca(z: torch.Tensor) -> np.ndarray:
    ...
    pixels = z[0, :, 0].cpu().float().reshape(C, -1).T.numpy()  # (h*w, C)
...
def pgd_attack_vae(
    ...
    """PGD attack on video VAE with single-frame input.

    images: (1, 3, 1, H, W) tensor in [-1, 1]
...
def visualize_attack(vae, original, adversarial, save_path, image_name=""):
    ...
    lat_ch_diff_raw = (z_adv[0, :3, 0] - z_orig[0, :3, 0]).abs().cpu().float()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PGD attack on CogVideoX VAE")
    parser.add_argument("--input_dir", type=str, default="resources/test-images")
    parser.add_argument("--output_dir", type=str, default="results/cogvideox_pgd")
    ...
    parser.add_argument("--image_size", type=int, default=480,
                        help="Image size (CogVideoX default is 480)")
    ...
    # Load CogVideoX VAE (16 latent channels, 3D video VAE)
    print(f"Loading CogVideoX VAE from {args.model_id} ...")
    vae = AutoencoderKLCogVideoX.from_pretrained(args.model_id, subfolder="vae")
